...
Status: Current Phase 1 viability boundary. This document describes the accepted private pilot; the [historical assessment](../evidence/historical/personal-use-viability-and-expansion.md) preserves earlier decision evidence separately.

## Viability decision

Neobrui is viable as a narrow, private, CSS-only kit for Ryan's personal projects. Phase 1 provides a stable `nbr` namespace, semantic tokens, Surface, Button, and Field Blocks, an ordered five-layer cascade contract, and product-owned theme state. It is not a public framework, published package, deployment target, or support promise.
...
1. Keep the current three-Block surface stable while the private pilot is evaluated.
2. Prefer a neutral example before adopting the kit in a live project.
3. Consider Stack and Cluster only when repeated layout needs justify a shared contract; define and test their API before implementation.
...
The pilot may be used in controlled local work under the repository's automated browser matrix. Public source, licensing, registry distribution, versioning, support commitments, and external user evidence remain future decisions. No claim of assistive-technology, operating-system, physical-device, or browser-UI zoom support is made until a dated manual run documents it.
