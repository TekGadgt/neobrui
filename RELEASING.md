...
The command checks the private manifest guard, exact version/tag, `next` prerelease classification, 12-file allowlist, checksums (SHA-256 and SHA-512 SRI), readable CSS and skill payload, and offline consumer installation. `report.json` contains stable semantic fields (source SHA, package metadata, file sizes/modes, archive digests, and consumer assertions) plus platform/tool facts; the timestamp is intentionally omitted so reports can be compared.
