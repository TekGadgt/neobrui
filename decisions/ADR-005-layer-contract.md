...
The Tailwind fixture imports the generated token CSS as an explicit application-owned input and selects `personal-light`; it has no neobrui plugin or adapter. Normal verification is non-mutating. The only verification capture is `CAPTURE_EVIDENCE=1 pnpm exec playwright test --project=chromium` (44 Chromium tests), which writes engine-labelled images to ignored `.evidence-cache/screenshots/`; committed `evidence/screenshots/` artifacts are read-only. `pnpm verify:clean` runs frozen install, all fixture builds, the full matrix, and Chromium capture, then asserts a clean tree and no owned preview process. Fixture `.astro/` generated state is ignored. See `evidence/spike-4-coexistence.md` for RED→GREEN cycles, computed-style matrix, builds, engine counts, and residual manual checks.
