...
Raised Surface and Button use one diagonal hard shadow: inline-axis offset, block-axis offset, zero blur, and semantic `--_nb-color-shadow`. The temporary `shadow.inline`, `shadow.block`, `shadow.pressInline`, and `shadow.pressBlock` roles are axis lengths/remaining pressed offsets, not complete shadow declarations. Fixture-owned values are `3px`, `4px`, `1px`, and `1px` respectively. The temporary internal schema contract accepts only `0` or a finite decimal with `px`, `rem`, `em`, `ch`, `ex`, `vw`, `vh`, `vmin`, or `vmax`; percentages, physical units, functions, colors, and multi-token declarations are rejected because box-shadow offsets require lengths and this recipe does not resolve arbitrary computed syntax. This is not public API stability.
