...
test.describe('Spike 2 blocks', () => {
  ...
  test('has native names and keyboard order', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Surface, Button, and Field blocks' })).toBeVisible();
    ...
    await expect(page.locator('label[for="block-email"]')).toBeVisible();
    await expect(page.locator('#block-email')).toHaveAttribute('aria-describedby', /block-email-description/);
    await page.keyboard.press('Tab');
    await expect(page.locator(':focus')).toHaveClass(/nbr-button/);
  });

  test('distinguishes surface hierarchy and keeps at least half quiet', async ({ page }) => {
    await expect(page.locator('.nbr-surface[data-nbr-level="quiet"]')).toHaveCount(1);
    await expect(page.locator('.nbr-surface[data-nbr-level="outlined"]')).toHaveCount(1);
    await expect(page.locator('.nbr-surface[data-nbr-level="raised"]')).toHaveCount(1);
    const quiet = await page.locator('.nbr-surface[data-nbr-level="quiet"]').count();
    const lowNoise = await page.locator('.nbr-surface:not([data-nbr-level="raised"])').count();
  ...
  test('raised surface and button use a nonzero inline hard-shadow offset', async ({ page }) => {
    const raised = page.locator('.nbr-surface[data-nbr-level="raised"]');
  ...
  test('button press has stable geometry and field invalid contract', async ({ page }) => {
    ...
    expect(await page.locator('#block-email').getAttribute('aria-invalid')).toBe('true');
    await expect(page.locator('#block-email-error')).toBeVisible();
    await expect(page.locator('#block-disabled')).toBeDisabled();
  });

  test('documented fixture theme boundary remaps consumed block hooks', async ({ page }) => {
    const boundary = page.locator('[data-nbr-fixture-theme="overrides"]');
    await expect(boundary).toHaveCount(1);
    await expect(boundary.locator('.nbr-surface[data-nbr-level="outlined"]')).toHaveCSS('background-color', 'rgb(239, 246, 255)');
    await expect(boundary.locator('.nbr-button').first()).toHaveCSS('background-color', 'rgb(124, 45, 18)');
    await expect(boundary.locator('.nbr-field input').first()).toHaveCSS('background-color', 'rgb(255, 247, 237)');
    await expect(boundary.locator('.nbr-surface[data-nbr-level="outlined"]')).toHaveCSS('border-top-color', 'rgb(30, 64, 175)');
  });

  test('200% text resize keeps content and controls available without overflow', async ({ page }) => {
    await page.evaluate(() => { document.documentElement.style.fontSize = '200%'; });
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true);
    for (const locator of [page.getByRole('button', { name: 'Native button' }), page.locator('#block-email'), page.locator('#block-disabled')]) {
  ...
  test('forced colors suppresses shadows and preserves non-color state cues', async ({ page, browserName }) => {
    ...
    expect(await page.locator('#block-disabled').evaluate(el => getComputedStyle(el).opacity)).not.toBe('1');
    expect(await page.locator('#block-email').evaluate(el => getComputedStyle(el).borderStyle)).toBe('dashed');
  });
  ...
  test('works without JavaScript and passes axe', async ({ browser, page }) => {
    ...
    await noJs.goto('http://127.0.0.1:4173/blocks/');
    await expect(noJs.getByRole('heading', { name: 'Surface, Button, and Field blocks' })).toBeVisible();
    await context.close();
  });
});
