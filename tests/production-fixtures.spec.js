...
test.describe('built CSS Modules and Astro production fixtures', () => {
  ...
  test('CSS Modules hashed local class and global block coexist', async ({ page }) => {
    ...
    const evidence = await page.evaluate(() => {
      ...
      return {
        ...
        blockBackground: getComputedStyle(button).backgroundColor,
        blockBorder: getComputedStyle(button).borderWidth,

      };
    });
    expect(evidence.classes.some(name => /^_panel_[a-zA-Z0-9]+_\d+$/.test(name))).toBe(true);
    expect(evidence.classes).toContain('nbr-surface');
    expect(evidence.panelPadding).toBe('16px');
    expect(evidence.blockBackground).toBe('rgb(143, 45, 45)');
    expect(evidence.blockBorder).toBe('2px');

  });

  test('Astro built route resolves global block and scoped application styles', async ({ page }) => {
    ...
    const evidence = await page.evaluate(() => {
      ...
      return {
        hasBlock: Boolean(document.querySelector('link[href*="/_astro/"]')),
    ...
    expect(evidence.hasBlock).toBe(true);
